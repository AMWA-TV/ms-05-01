...
Every NCA object has a name that is unique _within its containing block_ (see [**Identification ~link~**](#_rcilelyelwgb)), and every block constitutes a namespace for the objects it contains.
...
- The schema must import the schemas of all nested blocks - see [**Schema composition ~link~**](#_fbm27574bqse).
...
- The schema may specify the block class's signal flow, if signal paths are being used - see [**Signal paths ~link~**](#_y6cxs2q48h9).
- Every schema is identified by a **schema ID.**
- For schemas which are intended to be used across industry or across products, this **schema ID** value will be constructed in a way that is globally unique - a Global Schema ID (GSID) value - see [**Identification ~link~**](#_rcilelyelwgb).
...
NCA also allows blocks whose contents are dynamic at runtime, see [**Device configurability ~link~**](#_fxswj84o5yz).
...
The syntax of the GSID is defined in [**Identification ~link~**](#aute3teyjoks).
...
The housekeeping objects are called **managers** and are discussed in [**Managers ~link~**](#_96ljnmcmwsf6).
