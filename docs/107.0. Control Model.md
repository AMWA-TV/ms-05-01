...
The subscription mechanism is described further in [Events, Notifications and Subscriptions](110.0.%20Framework_Mechanisms.md#103-events-notifications-and-subscriptions).
...
4. A child class's inherited elements must retain the respective element IDs (see [Identification](109.0._Identification.html)) from its parent class.
...
- The **Blocks** subtree contains the definition of **ncaBlock** - see [Blocks](108.0._Device_Model.html#81-blocks).
...
- The **Managers** subtree contains definitions of classes that perform Device housekeeping functions. Each manager class may be instantiated at most once per NCA device. A few of them must be instantiated for NCA conformance - see [Base Device Schema](108.0._Device_Model.html#8221-base-device-schema).
